...
class M3FENDModel(torch.nn.Module):
    def __init__(self, emb_dim, mlp_dims, dropout, semantic_num, emotion_num, style_num, LNN_dim, domain_num, dataset):
        ...
        self.domain_num = domain_num
        self.dataset = dataset # Added dataset assignment
        self.gamma = 10
        self.memory_num = 10
        ...
        #    torch.Tensor(self.LNN_dim, self.semantic_num_expert + self.emotion_num_expert + self.style_num_expert)
        #).unsqueeze(0).cuda()

        total_experts = self.semantic_num_expert + self.emotion_num_expert + self.style_num_expert
        self.weight = torch.nn.Parameter(
            torch.empty(self.LNN_dim, total_experts).unsqueeze(0)
)
...
class Trainer():
    def __init__(self,
                 ...
                 style_num,
                 lnn_dim,
                 dataset,           # <-- añadir dataset
                 early_stop = 5,
                 epoches = 100,
                 **kwargs):               # <-- opcional pero útil
        self.lr = lr
        ...
        self.lnn_dim = lnn_dim
        self.dataset = dataset # <-- añadir dataset
    ...
    def train(self, logger = None):
        ...
        self.model = M3FENDModel(self.emb_dim, self.mlp_dims, self.dropout, self.semantic_num, self.emotion_num, self.style_num, self.lnn_dim, len(self.category_dict), dataset=self.dataset)
        if self.use_cuda:
